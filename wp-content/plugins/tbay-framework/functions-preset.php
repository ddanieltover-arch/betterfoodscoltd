...
if( ! function_exists( 'tbay_framework_init_redux' ) ) {
...
if( ! function_exists( 'tbay_framework_redux_scripts' ) ) {
...
if( ! function_exists( 'tbay_framework_redux_duplicate_preset' ) ) {
    function tbay_framework_redux_duplicate_preset() {
        // Verify nonce
        if ( ! isset( $_POST['nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['nonce'] ) ), 'tbay_framework_preset_nonce' ) ) {
            wp_send_json_error( array( 'message' => esc_html__( 'Security verification failed.', 'tbay-framework' ) ) );
            wp_die();
        }

        // Check permissions
        if ( ! current_user_can( 'manage_options' ) ) {
            wp_send_json_error( array( 'message' => esc_html__( 'Insufficient permissions.', 'tbay-framework' ) ) );
            wp_die();
        }

        // Sanitize input
        $title = isset( $_POST['title'] ) && is_string( $_POST['title'] ) ? sanitize_text_field( wp_unslash( $_POST['title'] ) ) : '';
        $preset = isset( $_POST['default_preset'] ) && is_string( $_POST['default_preset'] ) ? sanitize_key( $_POST['default_preset'] ) : '';
        ...
        $key = strtotime( 'now' );
        if ( ! empty( $title ) ) {
            $presets = get_option( 'tbay_framework_presets', array() );
            $key = strtotime( 'now' );
            $presets[ $key ] = $title;
            ...
            wp_send_json_success( array( 'preset_key' => $key ) );
        } else {
            wp_send_json_error( array( 'message' => esc_html__( 'Preset title is required.', 'tbay-framework' ) ) );
        }
...
if( ! function_exists( 'tbay_framework_redux_delete_preset' ) ) {
    function tbay_framework_redux_delete_preset() {
        ...
        if ( ! empty( $preset ) ) {
            ...
            wp_send_json_error( array( 'message' => esc_html__( 'Preset key is required.', 'tbay-framework' ) ) );
        }
...
if( ! function_exists( 'tbay_framework_redux_set_default_preset' ) ) {
    function tbay_framework_redux_set_default_preset() {
        ...
        // Sanitize input
        $default_preset = isset( $_POST['default_preset'] ) && is_string( $_POST['default_preset'] ) ? sanitize_key( $_POST['default_preset'] ) : '';
...
if( ! function_exists( 'tbay_framework_redux_save_new_preset' ) ) {
    function tbay_framework_redux_save_new_preset() {
        ...
        // Sanitize input
        $new_preset = isset( $_POST['new_preset'] ) && is_string( $_POST['new_preset'] ) ? sanitize_text_field( wp_unslash( $_POST['new_preset'] ) ) : '';
        ...
        if ( ! empty( $new_preset ) ) {
            ...
            wp_send_json_error( array( 'message' => esc_html__( 'Preset name is required.', 'tbay-framework' ) ) );
        }
...
if( ! function_exists( 'tbay_framework_redux_preset' ) ) {
