...
class Tbay_PostType_Megamenu {
    ...
    public static function custom_nav_update($menu_id, $menu_item_db_id, $args ) {
        ...
        if ( $post instanceof WP_Post ) {
            ...
            foreach ( $fields as $field ) {
                $key = 'menu-item-' . $field;
                if ( isset( $_REQUEST[ $key ][ $post->post_name ] ) ) {
                    $custom_value = sanitize_text_field( wp_unslash( $_REQUEST[ $key ][ $post->post_name ] ) );
                    update_post_meta( $menu_item_db_id, $field, $custom_value );
                }
            }

            // Invalidate cache
            wp_cache_delete( 'tbay_framework_sub_megamenus', 'tbay_framework' );
        }
    }

    public static function get_sub_megamenus(): array {
        $cache_key = 'tbay_framework_sub_megamenus';
        $cache_group = 'tbay_framework';
