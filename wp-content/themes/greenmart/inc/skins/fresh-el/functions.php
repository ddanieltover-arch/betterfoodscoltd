...
if (!function_exists('greenmart_tbay_private_size_image_setup')) {
    function greenmart_tbay_private_size_image_setup() {
        ...
        // Thumbnail size
        if (intval(get_option('thumbnail_size_w')) !== 360 || intval(get_option('thumbnail_size_h')) !== 187) {
            update_option('thumbnail_size_w', 360);
            update_option('thumbnail_size_h', 187);
        }

        // Medium size
        if ( intval(get_option('medium_size_w')) !== 540 || intval(get_option('medium_size_h')) !== 281 ) {
            update_option('medium_size_w', 540);
            update_option('medium_size_h', 281);
        }

        // Large size
        if ( intval(get_option('large_size_w')) !== 720 || intval(get_option('large_size_h')) !== 374 ) {
...
if (!function_exists('greenmart_tbay_autocomplete_suggestions')) {
    ...
    function greenmart_tbay_autocomplete_suggestions()
    {
        ...
        $post_type = isset($_REQUEST['post_type']) ? sanitize_key($_REQUEST['post_type']) : 'all';
        $number = isset($_REQUEST['number']) && is_numeric($_REQUEST['number']) 
        ? absint($_REQUEST['number']) 
        : 10;
        $number = max(1, min($number, 10));
        $product_cat = isset($_REQUEST['product_cat']) ? sanitize_key($_REQUEST['product_cat']) : '';
...
if (!function_exists('greenmart_tbay_list_theme_icons')) {
    function greenmart_tbay_list_theme_icons()
    {
        ...
        $theme_icons = array(
            ...
            'icon_menu_mobile'			=> 'icons icon-menu',
            'icon_wishlist_mobile'		=> 'icons icon-heart',
...
if (!function_exists('display_modes_active')) {
    function display_modes_active()
    {
        ...
        $active = (isset($_GET['enable_display_mode'])) ? (bool)$_GET['enable_display_mode'] : (bool)$active;
