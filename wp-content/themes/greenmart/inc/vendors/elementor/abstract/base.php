...
abstract class Greenmart_Elementor_Widget_Base extends Elementor\Widget_Base {
    ...
    protected function get_available_pages() {
        $cache_key = 'greenmart_available_pages';
    ...
    protected function get_available_on_sale_products() {
        $transient_key = 'greenmart_on_sale_products';
        ...
        $args = [
            'post_type'      => 'product',
            'post_status'    => 'publish',
            'posts_per_page' => apply_filters('greenmart_on_sale_products_limit', 100),
    ...
    protected function get_available_menus() {
        return greenmart_wpml_is_activated() ? $this->get_available_menus_wpml() : $this->get_available_menus_default();
    }
    ...
    protected function get_available_menus_default() {
        $cache_key = 'greenmart_available_menus';
    ...
    protected function get_available_menus_wpml() {
        global $sitepress;
        $cache_key = 'greenmart_menus_wpml_' . apply_filters('wpml_current_language', null);
        if (false === ($options = get_transient($cache_key))) {
            $menus = wp_get_nav_menus();
            $options = [];
    ...
    protected function get_product_categories() {
        $cache_key = 'greenmart_product_categories_all';
        ...
        if (false === $categories ) {
            ...
            $categories = is_wp_error($terms) ? [] : array_reduce($terms, function ($result, $term) {
                $result[greenmart_get_transliterate($term->slug)] = sprintf('%s (%d)', $term->name, $term->count);
                return $result;
            }, []);
    ...
    protected function get_woocommerce_tags() {
        $tags = get_transient('greenmart_woocommerce_tags');
        if (false === $tags) {
            $tags = array();
            $args = array(
                'order' => 'ASC',
                'hide_empty' => false,
                'number' => apply_filters('woocommerce_shortcode_products_per_page', 100),
            );
            ...
            set_transient('greenmart_woocommerce_tags', $tags, WEEK_IN_SECONDS);
        }
        return $tags;
    }
