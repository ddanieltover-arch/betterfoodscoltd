...
class Greenmart_Elementor_Custom_Image_List_Categories extends  Greenmart_Elementor_Carousel_Base{
    ...
    public function render_item_content($item, $attribute, $display_count_category) {
        ...
        // Handle custom link logic
        $url_category = $this->get_category_url($item, $category, $attribute);
        
            $this->render_item_type(
                $item['type'] ?? '',
                $url_category,
                $item['type_icon'] ?? '',
                $item['type_image'] ?? ''
    ...
    /**
     * Get category URL based on custom link settings
     *
     * @param array  $item      Category item data
     * @param string $category  Category slug
     * @param string &$attribute Reference to HTML attributes
     * @return string
     */
    private function get_category_url($item, $category, &$attribute) {
        $custom_link = !empty($item['custom_link']) ? $item['custom_link'] : [];
        $display_custom = !empty($item['display_custom']) && $item['display_custom'] === 'yes';

        if ($display_custom && !empty($custom_link['url'])) {
            $url = $custom_link['url'];
            if (!empty($custom_link['is_external']) && $custom_link['is_external'] === 'on') {
                $attribute .= ' target="_blank"';
            }
            if (!empty($custom_link['nofollow']) && $custom_link['nofollow'] === 'on') {
                $attribute .= ' rel="nofollow"';
            }
        } else {
            $url = get_term_link($category, 'product_cat');
            if (is_wp_error($url)) {
                $url = '#'; // Fallback if term link fails
            }
        }

        return apply_filters('products_category_widget_url', $url, $item);
    }
