...
class Greenmart_Elementor_Account extends Greenmart_Elementor_Widget_Base {
    ...
    protected function is_user_logged_in() {
        ...
        return self::$current_user->exists();
    }
    ...
    public function render_item_account() {
        ...
        do_action('greenmart_before_render_account', $settings);
        
        if (greenmart_tbay_get_theme() === 'organic-el') {
            $this->check_login_organic_el($settings['show_icon'], $settings['show_text_account'], $settings['text_after'], $settings['text_before'], $settings['icon_account']);
        } else {
            $this->check_login_fresh_el($settings['show_icon'], $settings['show_text_account'], $settings['text_after'], $settings['text_before'], $settings['icon_account']);
        }

        do_action('greenmart_after_render_account', $settings);
    }
    ...
    public function render_sub_menu() {
        ...
        $transient_key = 'greenmart_menu_account_' . md5($sub_menu_account);
