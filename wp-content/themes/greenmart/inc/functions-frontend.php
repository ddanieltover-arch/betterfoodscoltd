...
if (!function_exists('greenmart_get_header_id') ) {
	/**
	 * Get the ID of the current header layout.
	 *
	 * @return int|string Header ID or empty string if not found.
	 */
	function greenmart_get_header_id() {
	    $header     = apply_filters( 'greenmart_tbay_get_header_layout', 'default' );
	    $cache_key = 'greenmart_header_id_' . md5($header);
	    $header_id = (int) get_transient($cache_key);
	    ...
	    if (!$header_id) {
	        ...
	        $header_id = $query->have_posts() ? $query->posts[0] : '';
	        set_transient($cache_key, $header_id, YEAR_IN_SECONDS);
	    }
	    return $header_id;
	}
}
...
if (!function_exists('greenmart_get_footer_id') ) {
	function greenmart_get_footer_id() {
		...
		$cache_key = 'greenmart_footer_id_' . md5($footer);
		$footer_id = (int) get_transient($cache_key);
		...
		if (!$footer_id) {
			...
			$query = new WP_Query($args);
			$footer_id = $query->have_posts() ? $query->posts[0] : '';
			set_transient($cache_key, $footer_id, YEAR_IN_SECONDS);
		}
		return $footer_id;
	}
}
...
if ( ! function_exists( 'greenmart_tbay_get_menu_mobile_icon' ) ) {
	function greenmart_tbay_get_menu_mobile_icon( $ouput) {
		...
		if( $menu_option == 'smart_menu' ) {
			...
			$ouput 	.= '<a href="javascript:void(0);" class="btn btn-sm btn-danger mmenu-open">';
...
if ( ! function_exists( 'greenmart_get_social_html' ) ) {
	function greenmart_get_social_html($key, $value, $title, $link, $media) {
		...
		switch ($key) {
			...
			case 'twitter':
				$output = sprintf(
					'<a class="share-twitter greenmart-twitter" href="http://x.com/share?text=%s&url=%s" title="%s" target="_blank"><i class="fa fa-twitter"></i></a>',
					esc_attr( $title ),
					urlencode( $link ),
					urlencode( $title )
				);
...
if ( !function_exists('greenmart_tbay_back_to_top') ) {
    function greenmart_tbay_back_to_top(){
        ...
        if ( greenmart_tbay_get_config('back_to_top') ) { ?>
            <div class="tbay-to-top <?php echo esc_attr($tbay_header); ?>">
                ...
                <a class="text-skin wishlist-icon" href="<?php $wishlist_url = YITH_WCWL()->get_wishlist_url(); echo esc_url($wishlist_url); ?>"><i class="<?php echo greenmart_get_icon('icon_wishlist'); ?>" aria-hidden="true"></i><span class="count_wishlist"><?php $wishlist_count = yith_wcwl_count_products(); echo esc_attr($wishlist_count); ?></span></a>
                <?php } ?>
    ...
    add_action('elementor/theme/after_do_footer', 'greenmart_tbay_back_to_top', 10);
    add_action('greenmart_after_do_footer', 'greenmart_tbay_back_to_top', 10);
}

if (!function_exists('greenmart_logout_without_confirm')) {
    add_action('check_admin_referer', 'greenmart_logout_without_confirm', 10, 2);
    function greenmart_logout_without_confirm($action, $result)
    {
        /**
